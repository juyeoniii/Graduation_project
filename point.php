<?php 
	...
	try{
		...
		$stmt= mysqli_prepare($conn, "UPDATE userpoint SET userPoint = 50+userPoint WHERE userID = (?)");
		$state = mysqli_prepare($conn, "INSERT INTO userimages(userID, image) VALUES (?,?)");
		
		mysqli_stmt_bind_param($stmt, "s", $userID);
		mysqli_stmt_bind_param($state, "ss", $userID, $_FILES['pic']['name']);
		
		if(mysqli_stmt_execute($stmt) && mysqli_stmt_execute($state)){
